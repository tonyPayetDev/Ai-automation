      ...
      <div className="carousel-x overflow-x-auto snap-x snap-mandatory scroll-smooth -mx-6 px-6 pb-4 md:hidden">
        <div className="flex gap-4 w-max">
          {skillCategories.map((cat) => (
            <div
              key={cat.id}
              className="snap-start w-[78vw] max-w-[300px] flex-shrink-0 bg-[#0a0a0a] border border-white/5 rounded-2xl p-6 hover:border-[#B8965A]/25 transition-colors duration-500"
            >
              <div className="flex items-center gap-3 mb-5">
                <cat.icon size={18} className="text-[#B8965A]/70" strokeWidth={1.5} />
              ...
              <ul className="space-y-2">
                {cat.items.map((item, idx) => (
                  <li key={idx} className="text-xs text-gray-500 flex items-start gap-2">
                    <span className="text-[#B8965A]/50 mt-0.5">—</span>
                    {item}
                  </li>
                ))}
              </ul>
      ...
      <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {skillCategories.map((cat, index) => (
          <motion.div
            ...
            className="group bg-[#0a0a0a] border border-white/5 rounded-2xl p-7 hover:border-[#B8965A]/25 transition-all duration-500 hover:bg-[#0d0d0d]"
          >
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-white/5">
              <div className="w-9 h-9 rounded-xl bg-[#050505] border border-white/5 flex items-center justify-center text-[#B8965A]/60 group-hover:text-[#B8965A] group-hover:border-[#B8965A]/20 transition-all duration-400">
            ...
            <ul className="space-y-2.5">
              {cat.items.map((item, idx) => (
                <li key={idx} className="text-sm text-gray-500 flex items-start gap-2.5 group-hover:text-gray-400 transition-colors duration-300">
                  <span className="text-[#B8965A]/40 mt-0.5 flex-shrink-0">—</span>
                  {item}
                </li>
              ))}
            </ul>
