...
const AdminPanel: React.FC<AdminPanelProps> = ({ isOpen, onClose, onUpdateHero, activeOffer, onUpdateOffer }) => {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#0C0D18]/80 backdrop-blur-sm p-4">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="w-full max-w-5xl bg-[#0C0D18] border border-[#3DC4C2]/30 rounded-lg shadow-[0_0_50px_rgba(234,179,8,0.1)] overflow-hidden flex flex-col max-h-[90vh]">
                <div className="flex items-center justify-between p-4 border-b border-[#3DC4C2]/20 bg-zinc-900/50">
                    <div className="flex items-center gap-3">
                        <Lock className="w-5 h-5 text-[#3DC4C2]" />
                        <h2 className="text-xl font-bold text-white font-orbitron tracking-wider">ADMIN_CONSOLE // <span className="text-[#3DC4C2]">SYSTEM_MANAGER</span></h2>
                    </div>
                ...
                <div className="flex flex-1 overflow-hidden">
                    <div className="w-64 border-r border-white/10 bg-[#0C0D18]/50 p-4 hidden md:flex flex-col gap-2">
                        <button onClick={() => setActiveTab('offers')} className={`flex items-center gap-3 p-3 rounded text-left transition-all ${activeTab === 'offers' ? 'bg-[#3DC4C2] text-black font-bold' : 'text-gray-400 hover:bg-white/5'}`}>
                            <Layout size={18} /> OFFER_MANAGER
                        </button>
                        <button onClick={() => setActiveTab('upload')} className={`flex items-center gap-3 p-3 rounded text-left transition-all ${activeTab === 'upload' ? 'bg-[#3DC4C2] text-black font-bold' : 'text-gray-400 hover:bg-white/5'}`}>
                            <Upload size={18} /> UPLOAD_HERO
                        </button>
                        <button onClick={() => setActiveTab('generate')} disabled={!selectedImage} className={`flex items-center gap-3 p-3 rounded text-left transition-all ${activeTab === 'generate' ? 'bg-[#3DC4C2] text-black font-bold' : 'text-gray-400 hover:bg-white/5'} ${!selectedImage ? 'opacity-50 cursor-not-allowed' : ''}`}>
                    ...
                    <div className="flex-1 p-6 overflow-y-auto bg-zinc-950/50 relative">
                        ...
                        {activeTab === 'offers' && (
                          <div className="space-y-8 pb-12">
                            <div className="flex items-center justify-between border-b border-white/10 pb-4">
                              <h3 className="text-white font-bold flex items-center gap-2"><Settings size={18} className="text-[#3DC4C2]"/> CONFIGURATION_ACTIVE: {localOffer.name}</h3>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              <div className="space-y-4">
                                ...
                                <input 
                                  ...
                                  className="w-full bg-[#0C0D18] border border-white/10 p-3 rounded text-white font-mono text-sm focus:border-[#3DC4C2] outline-none" 
                                />
                                <input 
                                  value={localOffer.subtitle} 
                                  onChange={e => setLocalOffer({...localOffer, subtitle: e.target.value})}
                                  className="w-full bg-[#0C0D18] border border-white/10 p-3 rounded text-gray-400 font-mono text-sm focus:border-[#3DC4C2] outline-none" 
                                />
                              </div>
                              <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                  <input 
                                    ...
                                    className="bg-[#0C0D18] border border-white/10 p-3 rounded text-white font-mono text-sm" 
                                  />
                                  <input 
                                    ...
                                    className="bg-[#0C0D18] border border-[#3DC4C2]/30 p-3 rounded text-[#3DC4C2] font-mono text-sm" 
                                  />
                                </div>
                                <input 
                                  ...
                                  className="w-full bg-[#0C0D18] border border-white/10 p-3 rounded text-gray-500 font-mono text-xs" 
                                />
                              </div>
                            </div>
                            ...
                            <div className="space-y-4">
                              ...
                              {localOffer.problems.map((p, i) => (
                                <div key={i} className="flex gap-2">
                                  <input 
                                    ...
                                    className="flex-1 bg-[#0C0D18] border border-white/10 p-2 rounded text-gray-300 text-sm" 
                                  />
                              ...
                              <button onClick={() => setLocalOffer({...localOffer, problems: [...localOffer.problems, "Nouveau problème"]})} className="text-xs text-[#3DC4C2] flex items-center gap-1 hover:underline"><Plus size={12}/> Ajouter un point</button>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                              ...
                              <div className="p-4 bg-green-500/5 border border-green-500/20 rounded">
                                ...
                                <input 
                                  className="w-full bg-[#0C0D18] border border-white/10 p-2 rounded text-white mb-3 text-sm"
                                ...
                                <div className="space-y-2">
                                  {localOffer.solution1.items.map((it, i) => (
                                    <div key={i} className="flex gap-2">
                                      <input 
                                        className="flex-1 bg-[#0C0D18]/50 border border-white/5 p-2 rounded text-gray-400 text-xs"
                                        value={it}
                                        onChange={e => {
                                ...
                                <div className="space-y-2">
                                  {localOffer.solution2.steps.map((s, i) => (
                                    <div key={i} className="flex flex-col gap-1 border-b border-white/5 pb-2 mb-2 group relative">
                                      ...
                                      <input className="bg-[#0C0D18]/40 border border-white/5 p-1 rounded text-white font-bold text-xs" value={s.title} onChange={e => {
                                      ...
                                      <input className="bg-[#0C0D18]/40 border border-white/5 p-1 rounded text-gray-500 text-[10px]" value={s.desc} onChange={e => {
                                        const newSteps = [...localOffer.solution2.steps];
                        ...
                        {activeTab === 'upload' && (
                            <div className="h-full flex flex-col items-center justify-center border-2 border-dashed border-white/20 rounded-lg hover:border-[#3DC4C2]/50 transition-colors bg-white/5 py-12">
                                <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleFileChange} />
                                {selectedImage ? (
                                    <div className="text-center">
                                        <img src={selectedImage} alt="Selected" className="max-h-64 rounded shadow-lg border border-[#3DC4C2]/30 mb-4" />
                        ...
                        {activeTab === 'generate' && (
                            <div className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-xs text-[#3DC4C2] font-mono">SOURCE_INPUT</label>
                                        <div className="aspect-[9/16] bg-[#0C0D18] rounded border border-white/10 overflow-hidden relative">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="aspect-[9/16] bg-[#0C0D18] rounded border border-cyan-500/30 overflow-hidden relative flex items-center justify-center">
                                            {isGenerating ? (
                                ...
                                <div className="space-y-2">
                                    ...
                                    <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} className="w-full bg-[#0C0D18] border border-white/20 rounded p-3 text-sm text-gray-300 focus:border-[#3DC4C2] focus:outline-none font-mono h-24" />
                                </div>
