  ...
  <section id="experience" className="py-24 md:py-32 relative scroll-mt-20">
    ...
    <div className="max-w-4xl mx-auto px-6 lg:px-12">
      ...
      <div className="relative">
        {/* Vertical line */}
        <div className="absolute left-0 md:left-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-[#3DC4C2]/30 via-[#3DC4C2]/10 to-transparent md:-translate-x-px" />
        ...
        <div className="space-y-10">
          {experiences.map((exp, index) => (
            ...
            >
              {/* Dot */}
              <div className="absolute left-0 md:left-1/2 w-3 h-3 bg-[#0C0D18] border border-[#3DC4C2]/60 rounded-full md:-translate-x-1.5 mt-2 z-10" />

              <div className="pl-8 md:pl-0 md:w-1/2">
                <div className={`bg-[#171726] border border-white/5 rounded-2xl p-6 hover:border-[#3DC4C2]/20 transition-all duration-500 ${index % 2 === 0 ? 'md:mr-10' : 'md:ml-10'}`}>
                  <div className="mb-4">
                    <h3
                    ...
                    <div className="text-[#3DC4C2] text-sm font-medium mb-2">{exp.company}</div>
                  ...
                  <ul className="space-y-1.5 mb-4">
                    {exp.description.map((desc, i) => (
                      <li key={i} className="flex items-start gap-2 text-sm text-gray-500">
                        <span className="text-[#3DC4C2]/40 mt-1.5 flex-shrink-0 text-[8px]">◆</span>
                        {desc}
                      </li>
                    ))}
                  </ul>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {exp.technologies.map((tech) => (
                      <span
                        key={tech}
                        className="px-2 py-0.5 text-xs border border-white/5 bg-[#0C0D18] text-gray-600 rounded-full"
                      >
                        {tech}
                      </span>
                    ))}
